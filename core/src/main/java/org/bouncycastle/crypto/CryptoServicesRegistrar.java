...
public final class CryptoServicesRegistrar
{
    ...
    private static final String infoString = "BouncyCastle APIs (LTS edition) v2.73.14-SNAPSHOT";
    ...
    {
        final EntropySourceProvider baseEntropySourceProvider;

        if (NativeEntropySource.isAvailable())
        {
            baseEntropySourceProvider = new EntropySourceProvider()
            {
        ...
        {
            baseEntropySourceProvider = createBaseEntropySourceProvider();
        }
        ...
        if (Properties.isOverrideSet("org.bouncycastle.drbg.entropy_thread"))
        {
            ...
            return new EntropySourceProvider()
            {
                public EntropySource get(int bitsRequired)
                {
                    return new HybridEntropySource(entropyDaemon, baseEntropySourceProvider, bitsRequired);
                }
            };
        }
        else
        {
            return new EntropySourceProvider()
            {
                public EntropySource get(int bitsRequired)
                {
                    return new OneShotHybridEntropySource(baseEntropySourceProvider, bitsRequired);
                }
            };
        }
    }

        ...
        implements PacketCipher
{
    ...
    protected static void encryptBlock(int[] in, int[] out, int[][] KW, byte[] s, int ROUNDS)
    {
        ...
        // the final round's table is a simple function of S so we don't use a whole other four tables for it
        out[0] =
                (S[r0 & 255] & 255) ^ ((S[(r1 >> 8) & 255] & 255) << 8) ^ ((s[(r2 >> 16) & 255] & 255) << 16) ^ (s[(r3 >> 24) & 255] << 24) ^ KW[r][0];
        out[1] =
                (s[r1 & 255] & 255) ^ ((S[(r2 >> 8) & 255] & 255) << 8) ^ ((S[(r3 >> 16) & 255] & 255) << 16) ^ (s[(r0 >> 24) & 255] << 24) ^ KW[r][1];
        out[2] =
                (s[r2 & 255] & 255) ^ ((S[(r3 >> 8) & 255] & 255) << 8) ^ ((S[(r0 >> 16) & 255] & 255) << 16) ^ (S[(r1 >> 24) & 255] << 24) ^ KW[r][2];
        out[3] =
                (s[r3 & 255] & 255) ^ ((s[(r0 >> 8) & 255] & 255) << 8) ^ ((s[(r1 >> 16) & 255] & 255) << 16) ^ (S[(r2 >> 24) & 255] << 24) ^ KW[r][3];
    }
    ...
    protected static void encryptBlock(byte[] in, int inOff, byte[] out, int outOff, int[][] KW, byte[] s, int ROUNDS)
    {
        ...
        Pack.intToLittleEndian(C0, out, outOff);
    ...
    @Deprecated
    protected static final byte[] Si = {
    ...
    @Deprecated
    protected static void checkKeyLength(KeyParameter params, String exceptionMessage)
    {
        if (params != null)
        {
    ...
    @Deprecated
    protected static void ctrProcessBlock(byte[] counter, int[] counterIn, int[] counterOut, byte[] in, int inOff,
                                          byte[] out, int outOff,
    ...
    @Deprecated
    protected static void aeadLengthCheck(boolean forEncryption, int requiredLen, byte[] output, int outOff,
                                          int macSize)
            throws PacketCipherException
    {
        assert requiredLen >= 0;
        assert outOff >= 0;
        assert macSize > 0;

        if (forEncryption)
        {
            if (output.length - outOff - macSize < requiredLen)
            {

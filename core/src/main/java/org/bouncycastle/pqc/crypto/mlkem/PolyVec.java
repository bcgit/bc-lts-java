...
class PolyVec
{
    ...
    void compressPolyVec(byte[] rBuf, int rOff)
    {
        ...
        if (vec.length == 4)
        {
            ...
            for (int i = 0; i < vec.length; i++)
            {
                for (int j = 0; j < MLKEMEngine.KyberN / 8; j++)
                {
                    for (int k = 0; k < 8; k++)
                    {
                        /*t[k] = (short)
        ...
        else
        {
            ...
            for (int i = 0; i < vec.length; i++)
            {
                for (int j = 0; j < MLKEMEngine.KyberN / 4; j++)
                {
                    for (int k = 0; k < 4; k++)
                    {
                        /*t[k] = (short)
    ...
    void toBytes(byte[] buf, int off)
    {
        for (int i = 0; i < vec.length; i++)
        {
            vec[i].toBytes(buf, off + i * MLKEMEngine.KyberPolyBytes);
        }
    }
    ...
    void fromBytes(byte[] buf, int off)
    {
        for (int i = 0; i < vec.length; i++)
        {
            vec[i].fromBytes(buf, off + i * MLKEMEngine.KyberPolyBytes);
        }
    }

...
class MLKEMEngine
{
    ...
    final static int SeedBytes = KyberSymBytes * 2;
    ...
    MLKEMEngine(int k)
    {
        ...
        this.PolyVecBytes = k * KyberPolyBytes;
        this.IndCpaPublicKeyBytes = PolyVecBytes + KyberSymBytes;
        this.IndCpaSecretKeyBytes = PolyVecBytes;
        this.CipherTextBytes = PolyVecCompressedBytes + PolyCompressedBytes;
        this.SecretKeyBytes = IndCpaSecretKeyBytes + IndCpaPublicKeyBytes + 2 * KyberSymBytes;
    ...
    byte[][] kemEncrypt(MLKEMPublicKeyParameters publicKey, byte[] randBytes)
    {
        ...
        // SHA3-256 Public Key
        hash_H(encapKey, 0, encapKey.length, buf, KyberSymBytes);

        // SHA3-512( SHA3-256(RandBytes) || SHA3-256(PublicKey) )
        hash_G(buf, kr);

        // IndCpa Encryption
        byte[] outputCipherText = indCpa.encrypt(encapKey, 0, buf, 0, kr, KyberSymBytes);
    ...
    byte[] kemDecrypt(MLKEMPrivateKeyParameters privateKey, byte[] cipherText)
    {
        ...
        System.arraycopy(decapKey, SecretKeyBytes - 2 * KyberSymBytes, buf, KyberSymBytes, KyberSymBytes);

        byte[] kr = new byte[2 * KyberSymBytes];
        hash_G(buf, kr);

        int pkOff = IndCpaSecretKeyBytes;
        byte[] cmp = indCpa.encrypt(decapKey, pkOff, buf, 0, kr, KyberSymBytes);
        ...
        // if ciphertexts do not match, “implicitly reject”
        {
            ...
            xof.update(decapKey, SecretKeyBytes - KyberSymBytes, KyberSymBytes);

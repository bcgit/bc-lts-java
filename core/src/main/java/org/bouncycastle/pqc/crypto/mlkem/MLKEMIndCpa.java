...
class MLKEMIndCpa
{
    ...
    private static final int NUM_MATRIX_BLOCKS =
        (((12 * MLKEMEngine.KyberN / 8) << 12) / MLKEMEngine.KyberQ + SHAKE128_RATE) / SHAKE128_RATE;
    ...
    byte[][] generateKeyPair(byte[] d)
    {
        ...
        byte[] buf = new byte[2 * MLKEMEngine.KyberSymBytes];
        ...
        if (engine.getEta1() == 2)
        {
            for (int i = 0; i < K; i++)
            {
                secretKey.getVectorIndex(i).getNoiseEta2(xof, buf, MLKEMEngine.KyberSymBytes, nonce++);
            }

            for (int i = 0; i < K; i++)
            {
                e.getVectorIndex(i).getNoiseEta2(xof, buf, MLKEMEngine.KyberSymBytes, nonce++);
            }
        }
        else
        {
            for (int i = 0; i < K; i++)
            {
                secretKey.getVectorIndex(i).getNoiseEta3(xof, buf, MLKEMEngine.KyberSymBytes, nonce++);
            }

            for (int i = 0; i < K; i++)
            {
                e.getVectorIndex(i).getNoiseEta3(xof, buf, MLKEMEngine.KyberSymBytes, nonce++);
            }
        }
    ...
    byte[] unpackPublicKey(PolyVec publicKeyPolyVec, byte[] pk, int pkOff)
    {
        ...
        System.arraycopy(pk, pkOff + polyVecBytes, outputSeed, 0, MLKEMEngine.KyberSymBytes);
        return outputSeed;
    }
    ...
    void generateMatrixA(PolyVec[] aMatrix, byte[] seed, boolean transpose)
    {
        ...
        for (int i = 0; i < K; i++)
        {
            for (int j = 0; j < K; j++)
            {
                ...
                xof.update(seed, 0, MLKEMEngine.KyberSymBytes);
                ...
                int ctr = rejectionSampling(aMatrix[i].getVectorIndex(j), 0, MLKEMEngine.KyberN, buf, buflen);
    ...
    {
        short Q = (short)MLKEMEngine.KyberQ;

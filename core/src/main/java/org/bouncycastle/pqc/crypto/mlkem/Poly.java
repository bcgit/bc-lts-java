...
class Poly
{
    private final short[] coeffs = new short[MLKEMEngine.KyberN];
    ...
    void convertToMont()
    {
        final short f = (short)((1L << 32) % MLKEMEngine.KyberQ);
        for (int i = 0; i < MLKEMEngine.KyberN; i++)
        {
    ...
    void decompressPoly128(byte[] cBuf, int cOff)
    {
        ...
        {
            this.setCoeffIndex(2 * i + 0, (short)((((short)((cBuf[pos] & 0xFF) & 15) * MLKEMEngine.KyberQ) + 8) >> 4));
            this.setCoeffIndex(2 * i + 1, (short)((((short)((cBuf[pos] & 0xFF) >> 4) * MLKEMEngine.KyberQ) + 8) >> 4));
            pos += 1;
        }
    }
...
//                int t = (((c_j << 1) + (KyberEngine.KyberQ / 2)) / KyberEngine.KyberQ) & 1;
    ...
    {
        byte[] buf = new byte[2 * MLKEMEngine.KyberN / 4];
    ...
    {
        byte[] buf = new byte[3 * MLKEMEngine.KyberN / 4];
        prf(xof, seed, seedOff, nonce, buf);
        CBD.eta3(this, buf);
    }
    ...
    {
        xof.update(seed, seedOff, MLKEMEngine.KyberSymBytes);
        xof.update(nonce);

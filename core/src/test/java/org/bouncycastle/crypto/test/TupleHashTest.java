    ...
    extends DigestTest
{
    ...
    {
        // the requested length is the L parameter, so asking a 512 bit TupleHash for 32 bytes
        // is TupleHash128(X, 256, S), the sample value above, not a truncation of the 64 byte one.
        TupleHash tHash = new TupleHash(128, Strings.toByteArray("My Tuple App"), 512);
        ...
        tHash.doFinal(res, 0, res.length);

        isTrue("oops!", Arrays.areEqual(Hex.decode("E6 0F 20 2C 89 A2 63 1E DA 8D 4C 58 8C A5 FD 07 F3 9E 51 51 99 8D EC CF 97 3A DB 38 04 BB 6E 84"), res));

        tHash = new TupleHash(256, Strings.toByteArray("My Tuple App"), 256);
        ...
        // SP 800-185 sec. 5.1: changing the requested output length changes the output, so the shorter one is not a prefix of the longer.
        byte[] shortRes = new byte[32];
        byte[] longRes = new byte[64];

        tHash = new TupleHash(128, Strings.toByteArray("My Tuple App"));
        tHash.update(Hex.decode("000102"), 0, 3);
        tHash.doFinal(shortRes, 0, shortRes.length);

        tHash = new TupleHash(128, Strings.toByteArray("My Tuple App"));
        tHash.update(Hex.decode("000102"), 0, 3);
        tHash.doFinal(longRes, 0, longRes.length);

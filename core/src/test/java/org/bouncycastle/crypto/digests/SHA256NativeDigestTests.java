...
public class SHA256NativeDigestTests
        extends TestCase
{
    @Before
    public void setUp()
...
//        FipsStatus.isReady();
    ...
    public void testEncodedStateNoRetainedBlock() throws Exception
    {
        ...
        SavableDigest dig = SHA256Digest.newInstance();
        TestCase.assertTrue(dig instanceof SHA256NativeDigest);
        for (byte b : block)
        {
            dig.update(b);
        }
        ...
        SavableDigest restored = SHA256Digest.newInstance(state, 0);
        byte[] fromState = new byte[restored.getDigestSize()];
        TestCase.assertEquals(32, restored.doFinal(fromState, 0));

        byte[] res = new byte[dig.getDigestSize()];
        TestCase.assertEquals(32, dig.doFinal(res, 0));

        byte[] oracle = java.security.MessageDigest.getInstance("SHA-256").digest(block);

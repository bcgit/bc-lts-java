        ...
        if (algOid.on(IANAObjectIdentifiers.id_alg))
        {
            try
            {
                ...
                else if (key1Algorithm.contains("EC"))
                {
                    ...
                    if (publicKey != null)
                    {
                        ecPrivateKey = new ECPrivateKey(ECNamedCurveTable.getByOID(
                                                ASN1ObjectIdentifier.getInstance(ecPrivateKey.getParametersObject())).getCurve().getFieldSize(), ecPrivateKey.getKey(), ecPrivateKey.getParametersObject());
                    }

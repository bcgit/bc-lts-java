...
static inline void KF1600_StatePermute(uint64_t *state, const uint64_t *K) {
    int round;
...
static inline void keccak_absorb_buf(uint8x8_t *state, uint8_t *buf, size_t rateBytes, const uint64_t *K) {
    ...
    while (rateBytes > 0) {
        ...
        tmp = veor_u8(vld1_u8((void *) s), tmp); // eor into state
        vst1_u8((void *) s, tmp);
        s++;
        p += 8;
        rateBytes--;
    ...
    KF1600_StatePermute((uint64_t *) state, K);
}

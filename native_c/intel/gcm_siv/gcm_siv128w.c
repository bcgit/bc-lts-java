...
gcm_siv_err *processFourBlocksEnc(gcm_siv_ctx *ctx, uint8_t *in, uint8_t *out) {
...
gcm_siv_err *process_buffer_enc(gcm_siv_ctx *ctx,
    ...
    *read = *written = 0;
    ...
    if (ctx->bufBlockIndex == ctx->bufBlockLen) {
        ...
        gcm_siv_err *err = processFourBlocksEnc(ctx, ctx->bufBlock, out);
        if (err != NULL) {
            return err;
        }
...
gcm_siv_err *processFourBlocks_dec(gcm_siv_ctx *ctx, uint8_t *in, uint8_t *out) {
    ...
    *read = *written = 0;
    ...
    if (ctx->bufBlockIndex > 0 && ctx->bufBlockIndex + inlen >= ctx->bufBlockLen) {
        ...
        if (ctx->bufBlockIndex >= FOUR_BLOCKS) {
            ...
            gcm_siv_err *err = processFourBlocks_dec(ctx, ctx->bufBlock, out);
            if (err != NULL) {
                return err;
            }
            *written += FOUR_BLOCKS;
    ...
    if (ctx->bufBlockIndex == 0 && inlen >= ctx->bufBlockLen && outputLen >= FOUR_BLOCKS) {
        ...
        if (ctx->bufBlockIndex == 0 && inlen >= ctx->bufBlockLen) {
            ...
            gcm_siv_err *err = processFourBlocks_dec(ctx, in, out);
            if (err != NULL) {
                return err;
            }
            *written += FOUR_BLOCKS;

...
gcm_siv_pc_process_packet(bool encryption, uint8_t *key, size_t keysize, uint8_t *nonce, uint8_t *aad, size_t aadLen,
    ...
    deriveKeys(&H, roundKeys, key, (char *) nonce, keysize, &p_encrypt);
    gcm_siv_hasher_reset(&theAEADHasher);
    gcm_siv_hasher_reset(&theDataHasher);
    if (aad != NULL) {
        gcm_siv_hasher_updateHash(&theAEADHasher, &H, aad, aadLen, &theGHash);
    }
    gcm_siv_hasher_completeHash(&theAEADHasher, &H, &theGHash);
    if (encryption) {
        gcm_siv_hasher_updateHash(&theDataHasher, &H, p_in, inLen, &theGHash);

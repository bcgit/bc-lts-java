    ...
    if (*buf_pos == 0 && len >= 16) {
        ...
        while (len >= BLOCK_SIZE) {
            if (len >= 8 * BLOCK_SIZE) {
                ...
                if (!ccm_pc_incCtr(7, ctr, initialCTR, ctrMast, ctrAtEnd)) {
                    return false;
                }
                const __m128i c0 = _mm_xor_si128(*IV_le, _mm_set_epi64x(0, (int64_t) const_ctr));
                __m128i d0 = _mm_loadu_si128((__m128i *) &src[0 * 16]);
                ...
                if (!ccm_pc_incCtr(5, ctr, initialCTR, ctrMast, ctrAtEnd)) {
                    return false;
                }
                const __m128i c0 = _mm_xor_si128(*IV_le, _mm_set_epi64x(0, (int64_t) const_ctr));
                ...
                if (!ccm_pc_incCtr(3, ctr, initialCTR, ctrMast, ctrAtEnd)) {
                    return false;
                }
                const __m128i c0 = _mm_xor_si128(*IV_le, _mm_set_epi64x(0, (int64_t) const_ctr));

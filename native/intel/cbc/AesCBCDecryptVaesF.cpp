...
namespace intel {
    namespace cbc {
        ...
        size_t AesCBC128VaesFDec::processBlocks(unsigned char *in, uint32_t blocks, unsigned char *out) {
            unsigned char *outStart = out;

            __m512i fb512 = _mm512_broadcast_i32x4(feedback);

            while (blocks >= 16) {
                aes_cbc_dec_blocks_512b(in, out, fb512, feedback, roundKeys, 10, 16);
                in += CBC_BLOCK_SIZE_16;
                out += CBC_BLOCK_SIZE_16;
                blocks -= 16;
            }

            aes_cbc_dec_blocks_512b(in, out, fb512, feedback, roundKeys, 10, blocks);
        ...
        size_t AesCBC192VaesFDec::processBlocks(unsigned char *in, uint32_t blocks, unsigned char *out) {
            unsigned char *outStart = out;

            __m512i fb512 = _mm512_broadcast_i32x4(feedback);

            while (blocks >= 16) {
                aes_cbc_dec_blocks_512b(in, out, fb512, feedback, roundKeys, 12, 16);
                in += CBC_BLOCK_SIZE_16;
                out += CBC_BLOCK_SIZE_16;
                blocks -= 16;
            }

            aes_cbc_dec_blocks_512b(in, out, fb512, feedback, roundKeys, 12, blocks);
            out += (blocks * CBC_BLOCK_SIZE);
        ...
        size_t AesCBC256VaesFDec::processBlocks(unsigned char *in, uint32_t blocks, unsigned char *out) {
            unsigned char *outStart = out;

            __m512i fb512 = _mm512_broadcast_i32x4(feedback);

            while (blocks >= 16) {
                aes_cbc_dec_blocks_512b(in, out, fb512, feedback, roundKeys, 14, 16);
                in += CBC_BLOCK_SIZE_16;
                out += CBC_BLOCK_SIZE_16;
                blocks -= 16;
            }
            aes_cbc_dec_blocks_512b(in, out, fb512, feedback, roundKeys, 14, blocks);
            out += (blocks * CBC_BLOCK_SIZE);

...
//            System.out.println("},");
        ...
        throws Exception
    {
        Security.addProvider(new BouncyCastleProvider());
//
//        String[] names = {"data-0-09.bin",
//            "data-3-01.bin",
//            "data-2-03.bin",
//            "data-1-09.bin",
//            "data-4-02.bin"};
//        String[] ersName = {"data-0-09.bin.er", "data-3-01.bin.er", "data-2-03.bin.er", "data-1-09.bin.er", "data-4-02.bin.er"};
//
//        DigestCalculatorProvider digestCalculatorProvider = new JcaDigestCalculatorProviderBuilder().build();
//        DigestCalculator digestCalculator = digestCalculatorProvider.get(new AlgorithmIdentifier(NISTObjectIdentifiers.id_sha256));
//
...
//        {
//            dataList.add(new ERSInputStreamData(new FileInputStream("/tmp/bc/" + names[i])));
//        }
//
//        ERSDataGroup dataGroup = new ERSDataGroup(dataList);
//
//        ERSArchiveTimeStampGenerator ersGen = new ERSArchiveTimeStampGenerator(new JcaDigestCalculatorProviderBuilder().build().get(new AlgorithmIdentifier(NISTObjectIdentifiers.id_sha256)));
//
////        for (int i = 0; i != names.length; i++)
////        {
////            ersGen.addData(dataList.get(i));
////        }
//
//        ersGen.addData(dataGroup);
//
//        TimeStampRequestGenerator tspReqGen = new TimeStampRequestGenerator();
//
//        tspReqGen.setCertReq(true);
...
//
//        dataList = new ArrayList<ERSData>();
//        for (int i = 0; i != names.length; i++)
//        {
//            dataList.add(new ERSInputStreamData(new FileInputStream("/tmp/bc/" + names[i])));
//        }
//
//        dataGroup = new ERSDataGroup(dataList);
//
//        for (int i = 0; i != evs.size(); i++)
//        {
//            ERSEvidenceRecord ev = evs.get(i);
//            ev.validatePresent(dataGroup, new Date());
//
//            DigestCalculator newDigCalc = digestCalculatorProvider.get(new AlgorithmIdentifier(NISTObjectIdentifiers.id_sha512));
//
//            tspReq = ev.generateHashRenewalRequest(newDigCalc, dataGroup, tspReqGen);
//
//           tspReqGen = new TimeStampRequestGenerator();
//
//                    tspReqGen.setCertReq(true);
//
//                    infoGeneratorBuilder = new JcaSignerInfoGeneratorBuilder(digestCalculatorProvider);
//
//                    tsTokenGen = new TimeStampTokenGenerator(infoGeneratorBuilder.build(new JcaContentSignerBuilder("SHA256withRSA").setProvider(BC).build(origKP.getPrivate()), origCert), new SHA1DigestCalculator(), new ASN1ObjectIdentifier("1.2.3"));
//
//                    tsTokenGen.addCertificates(certs);
//
...
//                    try
//                    {
//                        tspResp = tsRespGen.generateGrantedResponse(tspReq, new BigInteger("23"), new Date());
//                    }
//                    catch (TSPException e)
//                    {
//                        tspResp = tsRespGen.generateRejectedResponse(e);
//                    }
//
//                ev = ev.renewHash(newDigCalc, dataGroup, tspResp);
//
//                ev.validatePresent(dataGroup, new Date());
//
//                FileOutputStream fOut = new FileOutputStream("/tmp/bc/group-data-" + i);
//                fOut.write(Streams.readAll(new FileInputStream("/tmp/bc/" + names[i])));
//                fOut.close();
//
//            FileOutputStream fOut2 = new FileOutputStream("/tmp/bc/group-data-" + i + ".ers");
//            fOut2.write(ev.getEncoded());
//            fOut2.close();
//              System.err.println(i);

        FileInputStream  fIn = new FileInputStream("/tmp/bc/data.txt");
        ERSEvidenceRecord ers = new ERSEvidenceRecord(new FileInputStream("/tmp/bc/data.txt.ers"), new JcaDigestCalculatorProviderBuilder().build());

        ERSData d = new ERSInputStreamData(fIn);

        ers.validatePresent(d, new Date());
    }
}

    ...
    extends TestCase
{
    ...
    // testKTSKeyTransAllowedContentAlgorithms and testKTSKeyTransMinimumTagSize omitted:
    // JceKTSKeyTransRecipientInfoGenerator(cert, "AES", 128) wraps via id_rsa_KEM
    // (1.2.840.113549.1.9.16.3.14, the RFC 6664 / X9.44 "KTS" scheme) - RSA-KEM is not
    // registered in this distribution's provider, matching the RSA-KEM tests disabled below.
    ...
    // testNtruKem omitted: NTRU is not part of this distribution.

    // testKemWithoutEncapsulationLength omitted: BIKE is not part of this distribution (no
    // Signature/KeyPairGenerator is registered for it, and there is no separate PQC provider
    // module here to supply one - PQC algorithms this distribution supports are folded directly
    // into the main provider instead).
     ...
     */
    // Disabled in this distribution: RSA-KEM (1.0.18033.2.2.4) is not registered in the provider.
...
//    }
    ...
    // Disabled in this distribution: helper for the RSA-KEM tests above.
...
//        assertTrue(Arrays.equals(enc, ContentInfo.getInstance(enc).getEncoded(ASN1Encoding.DER)));
    ...
    // Disabled in this distribution: the SM4 content-cipher key generator (1.2.156.10197.1.104.2) is not registered.

    ...
    extends TestCase
{
     ...
     * simply not processed in that position), but the PKIXCertPathReviewer
     * must surface the defect as a notification against the offending
     * certificate.
     */
     ...
     * revisit name constraints. PKIXCertPathReviewer drove the check from a loop bound of
     * index &gt; 0, which is the bound the CA-only steps want; index 0 is the end-entity
    ...
    private static boolean hasNotification(List notifications, String id)
    {
        ...
        {
            org.bouncycastle.pkix.util.ErrorBundle n = (org.bouncycastle.pkix.util.ErrorBundle)notifications.get(i);

...
The cert-path stacks — prov's `jce/provider` validator and pkix's `pkix/jcajce` revocation checker
and reviewer — cannot share package-private code: prov cannot see pkix, and
`org.bouncycastle.internal.*` is JPMS-concealed and OSGi-excluded, so it is invisible to the pkix
module/bundle (the same constraint behind the dual-located OID tables). When the same RFC 5280 rule
logic is needed in more than one stack, extract a **public, pure-ASN.1 engine class into core's
`org.bouncycastle.asn1.x509`** (visible everywhere via the core-into-prov bundling, and the package
is already exported so no `module-info` edit is needed) with a **public checked exception**, and
keep thin per-module wrappers that do the JCA extraction and rethrow as their local
...
`NameConstraintValidatorException` (sec. 6.1.4) and `PKIXCRLValidator` / `CRLValidatorException`
(sec. 6.3.3 CRL scope rules). JCA-bound duplication (CertPathBuilder plumbing, `Signature`/CRL
verification, store selectors) does **not** fit this pattern — sharing it would mean minting new
public JCA API in prov, a different trade-off.

Note the LTS-specific limits on this pattern. The legacy `org.bouncycastle.x509` stack and the
attribute-certificate path (`RFC3281CertPathUtilities`, `PKIXAttrCertPath*Spi`) are curated out of
this distribution, so upstream changes touching those copies have nothing to land on here. And
moving an *existing* public type is not available to us: relocating a class such as
`org.bouncycastle.jce.provider.PKIXPolicyNode` to a shared package and leaving a compatibility
subclass behind still moves its `protected` fields and its `implements` clause off the original
class, which `japi-compliance-checker` reports as high-severity removals. A shared *new* class is
additive and safe; re-homing a shipped one is not.

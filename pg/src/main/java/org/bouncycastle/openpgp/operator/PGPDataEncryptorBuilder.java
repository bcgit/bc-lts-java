...
public interface PGPDataEncryptorBuilder
{
     ...
     *
     * <p>
     * Declared as a default method so that pre-existing external implementations of this
     * interface remain binary/source compatible; the shipped {@code Bc}/{@code Jce} builders
     * override it. An implementation that has not been updated for v2 SEIPD (v6) AEAD will
     * throw {@link UnsupportedOperationException} rather than fail with {@code AbstractMethodError}.</p>
     *
     ...
     */
    default PGPDataEncryptor build(byte[] key, byte[] salt)
        throws PGPException
    {
        throw new UnsupportedOperationException(
            "v2 SEIPD (v6) AEAD build(key, salt) not implemented by " + getClass().getName());
    }
